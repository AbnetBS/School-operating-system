...
import { throttleByUser, IMPORT_LIMIT } from '../../../../lib/api/throttle.ts';
...
export const POST = route(async (request: NextRequest) => {
  ...
  if (mode === 'commit') {
    ...
    // Audit M3. A commit is the expensive half: measured at 3.2 s of database
    // time for 300 rows, against ~130 ms for a list read. Throttled after the
    // permission check so an unauthorised caller still gets 403, not 429, and
    // cannot use the limiter to probe what exists. Validation stays unlimited:
    // it writes nothing, and staff iterate on a preview until it is clean.
    const throttled = throttleByUser(ctx.user.userId, 'import:students', IMPORT_LIMIT);
