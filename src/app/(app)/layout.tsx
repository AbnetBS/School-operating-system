import { redirect } from 'next/navigation';
import { eq } from 'drizzle-orm';
import AppShell, { type NavItem } from '../../components/AppShell.tsx';
import { getAuthContext } from '../../lib/auth/context.ts';
import { schools } from '../../db/schema/core.ts';
import { todayIso, formatDate } from '../../lib/calendar/ethiopian.ts';
import { getSetting } from '../../lib/settings/service.ts';
import type { Permission } from '../../lib/auth/permissions.ts';
import { createTranslator } from '../../lib/i18n/index.ts';

export const dynamic = 'force-dynamic';

/**
 * Navigation is derived from permissions and enabled modules, so a user is
 * never shown a link they cannot use. This is convenience, not security — the
 * endpoints behind each link enforce the same rules independently.
 */
function buildNav(
  has: (p: Permission) => boolean,
  modules: Record<string, boolean>,
  t: ReturnType<typeof createTranslator>,
): NavItem[] {
  const items: NavItem[] = [];

  items.push({ href: '/dashboard', label: t('nav.dashboard'), icon: '◆' });

  if (modules.attendance && (has('attendance.take') || has('attendance.view'))) {
    items.push({ href: '/attendance', label: t('nav.attendance'), icon: '✓' });
  }
  if (modules.gradebook && (has('grade.view') || has('grade.enter'))) {
    items.push({ href: '/gradebook', label: t('nav.gradebook'), icon: '▦' });
  }
  if (modules.reportCards && (has('reportCard.view') || has('reportCard.generate'))) {
    items.push({ href: '/report-cards', label: t('nav.reportCards'), icon: '▣' });
  }
  if (modules.announcements && has('announcement.view')) {
    items.push({ href: '/announcements', label: t('nav.announcements'), icon: '❋' });
  }
  if (has('message.send')) {
    items.push({ href: '/messages', label: t('nav.messages'), icon: '✉' });
  }
  // Every signed-in person has a notification feed — it needs no permission.
  items.push({ href: '/notifications', label: t('nav.notifications'), icon: '◔' });
  // Finance covers two modules; either one on is enough to show the section,
  // and each page re-checks its own module and permission.
  if (
    (modules.fees || modules.payments)
    && (has('fee.view') || has('payment.view') || has('finance.report'))
  ) {
    items.push({ href: '/finance', label: t('nav.finance'), icon: '₿' });
  }
  // Analytics is permission-gated rather than module-gated: it reads whatever
  // modules the school has on, and shows nothing for those it does not.
  if (has('analytics.view')) {
    items.push({ href: '/analytics', label: t('nav.analytics'), icon: '◑' });
    items.push({ href: '/analytics/risk', label: t('nav.risk'), icon: '!' });
  }
  if (has('student.view')) {
    items.push({ href: '/students', label: t('nav.students'), icon: '☺' });
  }
  if (has('guardian.view')) {
    items.push({ href: '/guardians', label: t('nav.parents'), icon: '♥' });
  }
  if (has('academic.view')) {
    items.push({ href: '/academics', label: t('nav.setup'), icon: '▤' });
  }
  if (has('staff.view')) {
    items.push({ href: '/staff', label: t('nav.staff'), icon: '✦' });
  }
  // --- Operations (Group 8) --------------------------------------------------
  // Each entry needs BOTH the module switch and a permission. The page repeats
  // both checks: hiding a link is a convenience, never a control.
  if (modules.hr && (has('staffAttendance.view') || has('staffAttendance.take') || has('leave.view'))) {
    items.push({ href: '/hr', label: t('nav.staffAttendance'), icon: '◷' });
  }
  if (modules.library && has('library.view')) {
    items.push({ href: '/library', label: t('nav.library'), icon: '❏' });
  }
  if (modules.inventory && has('inventory.view')) {
    items.push({ href: '/inventory', label: t('nav.inventory'), icon: '▧' });
  }
  if (modules.maintenance && (has('asset.view') || has('maintenance.report'))) {
    items.push({ href: '/facilities', label: t('nav.maintenance'), icon: '✂' });
  }
  if (modules.transport && has('transport.view')) {
    items.push({ href: '/transport', label: t('nav.transport'), icon: '⛟' });
  }
  if (modules.documents && has('document.view')) {
    items.push({ href: '/documents', label: t('nav.documents'), icon: '❐' });
  }
  if (has('event.view')) {
    items.push({ href: '/calendar', label: t('nav.calendar'), icon: '▦' });
  }
  if (has('audit.view')) {
    items.push({ href: '/audit', label: t('nav.audit'), icon: '⏱' });
  }
  if (has('school.manage') || has('notification.manageTemplates')) {
    items.push({ href: '/settings', label: t('nav.settings'), icon: '⚙' });
  }

  return items;
}

export default async function AppLayout({ children }: { children: React.ReactNode }) {
  const ctx = await getAuthContext();
  if (!ctx) redirect('/login');

  // Parents and students belong in their portals, not the staff application.
  if (ctx.has('portal.parent') && !ctx.has('student.view')) redirect('/portal/parent');
  if (ctx.has('portal.student') && !ctx.has('student.view')) redirect('/portal/student');

  const [school] = await ctx.db
    .select({ name: schools.name, nameAm: schools.nameAm })
    .from(schools)
    .where(eq(schools.id, ctx.schoolId))
    .limit(1);

  const [modules, localeSettings] = await Promise.all([
    getSetting(ctx.db, ctx.schoolId, 'modules'),
    getSetting(ctx.db, ctx.schoolId, 'locale'),
  ]);

  const today = todayIso(localeSettings.timezone);
  const t = createTranslator(ctx.locale);
  const nav = buildNav((p) => ctx.has(p), modules as unknown as Record<string, boolean>, t);

  const schoolName =
    ctx.locale === 'am' && school?.nameAm ? school.nameAm : (school?.name ?? 'School');

  return (
    <AppShell
      nav={nav}
      userName={ctx.displayName()}
      roleLabel={ctx.roleKeys.map(humanRole).join(', ') || 'Staff'}
      schoolName={schoolName}
      ethiopianDate={formatDate(today, { calendar: 'ethiopian', locale: ctx.locale })}
      gregorianDate={formatDate(today, { calendar: 'gregorian', locale: ctx.locale })}
    >
      {children}
    </AppShell>
  );
}

function humanRole(key: string): string {
  return key
    .split('_')
    .map((part) => part.charAt(0).toUpperCase() + part.slice(1))
    .join(' ');
}
