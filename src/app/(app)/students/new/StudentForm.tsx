...
} from '../../../../components/CustomFieldInputs.tsx';
...
export default function StudentForm({
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    // Only fields this school has actually defined are sent; the server
    // validates against the same definitions and rejects anything else.
    if (customFields.length > 0) {
      payload.customFields = readCustomFields(formData, customFields);
    }
        ...
        <Section title="Additional information">
          <div className="grid gap-3 sm:grid-cols-3">
            {customFields.map((def) => {
              ...
              // The API reports these as `customFields.<key>`.
              ...
              if (def.fieldType === 'boolean') {
                return (
                  <label key={def.key} className="tap-target flex items-center gap-2 sm:mt-6">
                    <input
                      type="checkbox"
                      name={name}
