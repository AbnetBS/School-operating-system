...
import { resolveClientIp, UNKNOWN_CLIENT_IP } from './clientIp.ts';
...
export async function buildContext(
  ...
  try {
    ...
    // Resolved through the trusted-proxy rules rather than read raw, so an
    // audit record cannot be stamped with an attacker-chosen address.
    const resolved = resolveClientIp(h);
    ipAddress = resolved === UNKNOWN_CLIENT_IP ? null : resolved;
  } catch {
