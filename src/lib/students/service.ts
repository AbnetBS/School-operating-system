...
export type StudentListRow = {
  ...
  /**
   * Only populated when the caller asks for it (`withCustomFields`). The list
   * screens do not need it, and a JSONB column on every row of a 400-pupil
   * school is real bytes over the wire for no benefit.
   */
...
export async function listStudents(
  ...
  const [rows, totals] = await Promise.all([
    db
      .select({
        ...
        ...(options.withCustomFields ? { customFields: students.customFields } : {}),
      })
      .from(students)
  ...
  // The JSONB column arrives as `unknown`; narrow it once here rather than at
  // every call site. A non-object value would mean a corrupt row, so it is
  // treated as "no custom values" rather than trusted.
  const typedRows = rows.map((row) => {
    const raw = (row as { customFields?: unknown }).customFields;
    return {
      ...row,
      ...(options.withCustomFields
        ? {
            customFields:
  ...
  return { rows: typedRows, total: totals[0]?.total ?? 0 };
}
...
export async function createStudent(
  ...
  // Custom fields are validated against this school's own definitions. An
  // undefined key is rejected rather than stored: see lib/customFields.
...
export async function updateStudent(
  ...
  for (const [key, value] of Object.entries(patch)) {
    ...
    // `customFields` is a whole object rather than a scalar column, and must
    // be validated against the school's definitions before it can be stored.
    if (key === 'customFields') {
      updates[key] = await resolveCustomFieldValues(ctx.db, ctx.schoolId, 'student', value);
      continue;
    }
