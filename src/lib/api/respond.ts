...
export function handleApiError(error: unknown): NextResponse {
  ...
  if (isDomainError(error) && error.status >= 400 && error.status < 500) {
    // A domain error may carry per-field messages — custom-field validation
    // reports against the specific field that failed. Only a plain
    // string→string map is forwarded, so an unexpected shape cannot leak an
    // internal object into the response body.
    ...
    if (raw && typeof raw === 'object' && !Array.isArray(raw)) {
      ...
      if (Object.keys(clean).length > 0) fields = clean;
    }

    return NextResponse.json(
      { error: error.message, ...(fields ? { fields } : {}) } satisfies ApiErrorBody,
      { status: error.status },
    );
  }
