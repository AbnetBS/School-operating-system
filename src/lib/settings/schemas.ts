...
/**
 * Lending policy. Every number here is a school decision, not a system rule:
 * a primary school lending picture books for a week and a secondary school
 * lending reference texts for a term both configure it here.
 */
export const librarySettingsSchema = z.object({
  /** Default loan length in days. */
  loanDays: z.number().int().min(1).max(365).default(14),
  /** How many items one borrower may hold at once. */
  maxLoansPerBorrower: z.number().int().min(1).max(50).default(3),
  /** How many times a loan may be extended. */
  maxRenewals: z.number().int().min(0).max(10).default(2),
  /**
   * Fine per day overdue, in cents. Zero means the school does not fine,
   * which is the common case in Ethiopian schools — hence the default.
   */
  finePerDayCents: z.number().int().min(0).max(100_000).default(0),
  /** Days before the due date to send a reminder. */
  reminderDaysBefore: z.number().int().min(0).max(30).default(2),
  /** Prefix for generated accession numbers. */
  accessionPrefix: z.string().trim().max(16).default('ACC'),
});
export type LibrarySettings = z.infer<typeof librarySettingsSchema>;

// ---------------------------------------------------------------------------
// Operations: leave and stock (Group 8)
// ---------------------------------------------------------------------------

export const operationsSettingsSchema = z.object({
  /**
   * Which weekdays count as working days when computing leave duration.
   * 0 = Sunday. Ethiopian schools commonly work Monday-Friday, some also
   * Saturday morning — so this is configured, never assumed.
   */
  workingDays: z
    .array(z.number().int().min(0).max(6))
    .min(1, 'A school must have at least one working day.')
    .default([1, 2, 3, 4, 5]),
  /** Whether a leave request needs approval before it takes effect. */
  leaveRequiresApproval: z.boolean().default(true),
  /** Warn when stock falls to this multiple of the reorder level. */
  lowStockWarningFactor: z.number().min(1).max(5).default(1),
  /** Default staff attendance status when a day is opened for marking. */
  defaultStaffAttendanceStatus: z
    .enum(['present', 'absent'])
    .default('present'),
});
...
// ---------------------------------------------------------------------------
// Risk and early warning (Group 9)
// ---------------------------------------------------------------------------

/**
 * Thresholds for the early-warning system.
 *
 * Every signal can be switched off, and every threshold is a number the school
 * chooses. A school that considers 85% attendance acceptable and one that
 * considers 92% acceptable must both get a system that agrees with them.
 *
 * The weights decide how much each signal contributes to the overall score.
 * They are deliberately plain numbers rather than a hidden model: a head
 * teacher must be able to read this configuration and predict the output.
 */
export const riskSettingsSchema = z.object({
  enabled: z.boolean().default(true),

  attendanceEnabled: z.boolean().default(true),
  /** Attendance at or below this percentage raises a signal. */
  attendanceThresholdPercent: z.number().min(0).max(100).default(85),
  attendanceWeight: z.number().min(0).max(100).default(30),

  consecutiveAbsenceEnabled: z.boolean().default(true),
  consecutiveAbsenceDays: z.number().int().min(1).max(60).default(3),
  consecutiveAbsenceWeight: z.number().min(0).max(100).default(25),

  academicEnabled: z.boolean().default(true),
  /** An average at or below this percentage raises a signal. */
  academicThresholdPercent: z.number().min(0).max(100).default(50),
  academicWeight: z.number().min(0).max(100).default(25),

  /** A drop of this many points between terms raises a signal. */
  declineEnabled: z.boolean().default(true),
  declinePoints: z.number().min(1).max(100).default(15),
  declineWeight: z.number().min(0).max(100).default(20),

  /**
   * Whether unpaid fees may contribute to a pupil's risk score.
   *
   * Defaults to FALSE and is deliberately separate from the rest. A child is
   * not "at risk" because their family is late paying, and mixing a financial
   * signal into a pastoral score invites exactly the wrong intervention.
   * A school may switch it on, but must do so knowingly.
   */
  financeEnabled: z.boolean().default(false),
  financeWeight: z.number().min(0).max(100).default(10),

  /** Score at or above which a pupil is listed as needing attention. */
  attentionScore: z.number().min(1).max(200).default(40),
});
