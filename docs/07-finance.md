...
Recorded after the module was built and tested, so the next person does not
have to rediscover these.

### The NULLS DISTINCT trap (migration 0010)

`applyFeeStructure` is safe to repeat because a unique index on
`(school_id, student_id, fee_structure_id, term_id, installment_number)`
lets `onConflictDoNothing` skip anyone already charged.

That index silently failed for every fee that is **not** per-term. `term_id`
is NULL for a one-off, monthly or custom fee, and under the SQL default two
NULLs never compare equal — so the index did not match, no conflict was
detected, and a second application billed every pupil a second time.

The termly path was tested and passed, which is exactly why this survived.
Migration 0010 rebuilds the index with `NULLS NOT DISTINCT` and clears the
duplicates it had already produced, skipping any charge with money against it.

Drizzle's index builder cannot express `NULLS NOT DISTINCT`, so that index is
maintained by hand in SQL. Anything regenerated by `drizzle-kit` will drop the
clause — check migration 0010 before trusting a generated file.

### PGlite serialises transactions

PGlite runs transactions strictly one after another; two overlapping ones never
interleave. A behavioural concurrency test therefore passes whether or not the
code locks anything, and proves nothing about the PostgreSQL server this
deploys to.

`tests/finance.test.ts` handles this by asserting on the SQL itself — the
charge SELECT and the receipt-counter SELECT must both carry `FOR UPDATE`.
Removing either lock fails a test.

### AuthError in a Server Component

`resolvePortalStudent` throws `AuthError(404)`. An API route turns that into a
JSON 404, but a React Server Component has no such handler and it surfaces as
a 500 — the page looks broken rather than absent. Portal pages go through
`src/app/portal/resolve-student.ts`, which converts that one case to Next's
`notFound()` and re-throws everything else.

### Services do not self-authorise

`listStudentCharges`, `getStudentBalance` and `listPayments` take a student id
and filter by school, but do **not** check the caller may see that pupil. That
is deliberate — the portal reaches them through a relationship, with no finance
permission at all — but it means every caller must first establish access via
`ctx.requireStudentAccess(id)` or `resolvePortalStudent`. A new caller that
forgets is a data leak, and the doc comments on those functions say so.
