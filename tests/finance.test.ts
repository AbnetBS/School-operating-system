...
before(async () => {
  db = await getDb();
  clearHandlers();
  // Unique codes per run, and the schools are removed afterwards, so this
  // file can be run repeatedly against the same database. With fixed codes it
  // passed only against a freshly migrated datadir and failed on every rerun.
  const stamp = Date.now();
  await seedSchool(`fina-${stamp}`, A);
  await seedSchool(`finb-${stamp}`, B);
});
